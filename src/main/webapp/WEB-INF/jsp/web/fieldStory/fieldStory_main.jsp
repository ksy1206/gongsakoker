...
// 	var member_no = $("#myList_member_no").val();
	params = "sido="+sido+"&gugun="+gugun+"&list_select="+list_select+"&search_choice="+search_choice+"&search_content="+search_content+"&pageNum="+pageNum;
    $.ajax({
        type        : "POST" 
      , async       : false
      ...
      , success     : function(data) {
    	  ...
    	  $.each(data.fieldStory.rows, function(index, fieldStory){
			...
			html += "</div>"
			html += "<div class='content_person' style='font-size: 15px;'>"+fieldStory.koker_name+"</div>";
    	  ...
    	  var check = false;
    	  var html2 = "";
    	  html2 += "<p style='text-align:center;'>";
    	  
    	  if(data.fieldStory.page_start > 1){
    		  html2 += "<a href='#' onclick='pagging_move("+(Number(data.fieldStory.currentPage)-Number(1))+")'>[이전]</a>";		
    	  }
    	  
    	  for(i=data.fieldStory.page_start; i<=data.fieldStory.totalPage; i++){
    		  if(check == false){
    			  if(i == $("#pageNum").val()){
    				  html2 += "<a href='#' style='color:#21b4d6; font-weight:600;' onclick='pagging_move("+i+")'>"+i+"</a>";
    			  } else {
    				  html2 += "<a href='#' onclick='pagging_move("+i+")'>"+i+"</a>";
    			  }
...
//       					html2 += "<li><a href='#' onclick='pagging_move("+(Number(data.fieldStory.currentPage)+Number(1))+")>[다음]</a></li>"
      					html2 += "<a href='#' onclick='pagging_move("+(Number(data.fieldStory.currentPage)+Number(1))+")'>[다음]</a>"
      				}
      			  }
    		  }  
    	  }
...
//내가 쓴 글 리스트 불러오기
function fieldStory_myList(){
	var member_no = $("#session_member_no").val();
	location.href="<%=cp%>/fieldStory_web/fieldStory_myList.do?member_no="+member_no;
}

//상세페이지 이동
function details(fs_no){
	var member_no = $("#session_member_no").val();
	location.href="<%=cp%>/fieldStory_web/details.do?fs_no="+fs_no+"&member_no="+member_no;
}
</script>
<script>
...
<!-- 	<input type="hidden" id="myList_member_no"> -->
	<input type="hidden" id="my_fieldStory_list">
	
	<div class="board_notice"  style="box-shadow:0 4px 2px -2px #d7d7d7;">
		    ...
		    <div style="line-height: 48px;">
				...
				<div class="board_search">
					<div class="search-wrapper">
						<form>
							<select id="search_choice" style="padding: .5em 0em .5em 1em;">
	...
	<div class="board_content">
		<div class="board_post_icon">
			<c:if test="${sessionScope.sessionData.memberInfo.getMember_no() != null && sessionScope.sessionData.memberInfo.getMember_no() != ''}">
				<span onclick="fieldStory_myList()" class="mypost" style="cursor:pointer;">내가 쓴 글<img src="<%=cp %>/img/local/mypost_icon.png" style="width:8%;vertical-align:middle;padding-left: .5em;"></span>
			</c:if>
		...
		<form>
		  	<div id="list_div" class="board_content" style="margin:0 auto; text-align:center;">
		  		<div class="container_title">
		  			<div class="content_local content_local_top">지역</div>
		  			<div class="content_title content_title_top" style="text-align:center;">제목</div>
		  		...
		  		<div id="content_list_div">
			  		<div style="text-shadow:none; font-weight:normal !important; color:#666; font-size:14px;margin:20px 33%;"> 검색 중</div>
		  		</div>
		    </div>
		</form>		
	</div>
