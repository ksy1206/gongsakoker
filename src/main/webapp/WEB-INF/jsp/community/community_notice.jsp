...
<div data-role="page" class="board_page">
	<div data-role="header" data-position="fixed" class="board_header">
		...
		<h1 style="font-size: 14px; font-weight: normal !important; text-shadow: none !important;">커뮤니티 공지사항</h1>
	</div>
	<div data-role="collapsible-set">
		<c:forEach var="data" items="${communityNotice_list}">
		    <div data-role="collapsible" data-collapsible="true" class="board_list">
			        <h3>
						<p>${data.c_s_title}</p>
						<p class="list_gray" style="font-size:12px;">${data.c_s_insert_date}</p>
					</h3>
		        <textarea readonly="readonly">${data.c_s_content}</textarea>
				<p class="list_gray">${data.c_s_insert_date}</p>
		    </div>
		</c:forEach>
	</div>
</div>
