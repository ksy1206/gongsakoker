...
<style>
	p{margin:1em 0;}
	.list_num li{margin:0 1% !important;}
</style>
...
// 	var member_no = $("#myList_member_no").val();
    ...
    $.ajax({
        type        : "POST" 
      , async       : false
      , url         : "/admin/search_fieldStory.do"
      , data        : params
      , dataType    : "json"
      , timeout     : 30000  
      , cache       : false    
      ...
      , success     : function(data) {
    	  ...
    	  $.each(data.fieldStory.rows, function(index, fieldStory){
			...
			if((fieldStory.title).length > 25){
				html += "<div class='content_title'><a href='#'>"+(fieldStory.title).substring(0,25)+"...</a>";	
			} else {
			...
			html += "</div>"
			if((fieldStory.member_id).length > 5){
				html += "<div class='content_person'>"+(fieldStory.member_id).substring(0,5)+"...</div>";
			} else {
    	  ...
    	  var check = false;
    	  var html2 = "";
    	  html2 += "<ul style='text-align: center;'>";
...
function details(fs_no){
	location.href="<%=cp%>/admin/admin_fieldStoryDetails.do?fs_no="+fs_no;
}
</script>
