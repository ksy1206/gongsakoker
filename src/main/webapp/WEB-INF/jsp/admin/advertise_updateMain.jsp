...
<body>
	...
	<c:forEach items="${list}" var="list">
		...
		<c:if test="${list.board_complete == 'y' }">
			<form action="advertise_update.do" method="post" id="avertiseUpdate" name="avertiseUpdate">
				...
				<table style="width:100%;">
					...
					<tr>
						<td colspan="2" style="text-align:center; padding:3em 0 1em 0;">
							<input type="button" value="수정"  onclick="javascript:advertiseUP();" style="font-family:'GodoM'; font-size:14px; color:#fff; border:solid #29A9C8 1px; background-color:#29A9C8; border-radius:5px; padding:.5em 2em; margin-right:1em;"/>
							<input type="button" value="취소"  onclick="location.href='admin_advertise.do?pageNum=1'" style="font-family:'GodoM'; font-size:14px; color:#29A9C8; border:solid #29A9C8 1px; background-color:#fff; border-radius:5px; padding:.5em 2em;"/>
						</td>
					</tr>
				</table>
			</form>
		...
		<c:if test="${list.board_complete == 'n' }">
			<form action="advertise_insert.do" method="post" id="avertiseInsert" name="avertiseInsert">
