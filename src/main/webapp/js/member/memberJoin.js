                    ...
                    $.ajax({
                      ...
                      , success     : function(data) {
                                 ...
                                 if(data.member_id_chk== "N"){
                                     $( "#dialog_txt").text( "아이디 사용가능합니다." );
                                     $( "#member_id_chk").val("Y");
                                     
                                } else{
                                     $( "#dialog_txt").text( "아이디가 중복되었습니다." );
                                     $( "#member_id_chk").val("N");
...
//                 if(hp== null||hp== ""||hp== undefined ||hp=="휴대폰번호를 입력해주세요." ){
...
//                     
//                      if(hp.search( "-") == "-1" ){
//                          $( "#dialog_txt").text( "000-0000-0000 형식으로 입력해주세요.");
//                          $( "#popupDialog").popup( "open");
//                           return false;
//                     } else{
//                          
...
//                 params = "name=" +name+"&member_id=" +member_id+"&hp01=" +hp01+"&hp02=" +hp02+"&hp03=" +hp03+"&password=" +password;
                ...
                class_code = $('input:radio[name="chk_info"]:checked' ).val();
...
//                            var call_tel03 = call_tel_arr[2];
                      ...
                      if(koker_name== null||koker_name== ""||koker_name ==undefined ||koker_name=="업체명을 입력해주세요." ){
                           $( "#dialog_txt").text( "업체명을 입력해주세요.");
                           $( "#popupDialog").popup( "open");
                           $( "#company_name").focus();
                            return false;
                }
                     
//                      if(call_tel== null||call_tel== ""||call_tel == undefined ||call_tel=="업체 연락가능한 번호를 입력해주세요." ){
//                           $( "#dialog_txt").text( "업체 연락가능한 번호를 입력해주세요.");
//                           $( "#popupDialog").popup( "open");
//                           $( "#company_tel").focus();
//                            return false;
//                } else{
//
//                            if(call_tel.search( "-") == "-1" ){
//                                $( "#dialog_txt").text( "000-0000-0000 형식으로 입력해주세요.");
//                                $( "#popupDialog").popup( "open");
//                                 return false;
//                           } else{
...
//                      }
                     ...
                     $( "input[name='category']" ).each(function (index){
                           ...
                           categoryVal  = categoryVal.substring(0, categoryVal.length - 1);
                            ...
                            if(categoryVal.length==0){
                                $( "#dialog_txt").text( "전문분야를 1개 이상 선택해주세요.");
                                $( "#popupDialog").popup( "open");
                                 return false;
                           }
                      ...
                      if(class_code == "03"){
                            ...
                            if(area== null||area== ""||area == undefined ||area=="default" ){
                                $( "#dialog_txt").text( "도지역을 선택해주세요.");
                                $( "#popupDialog").popup( "open");
                                
                                 return false;
                     }
                            if(city== null||city== ""||city == undefined ||city=="default" ){
                                $( "#dialog_txt").text( "시/군/구 지역을 선택해주세요.");
                                $( "#popupDialog").popup( "open");
	...
	//                              $("#dialog_txt").text("사업자등록증을 첨부해주세요.");
	//                              $("#popupDialog"). popup("open");
	//                              return false;
	                                memberJoin();
	                    		}
             ...
             contentType : "application/x-www-form-urlencoded;charset=UTF-8" ,
                success : function(data){
                            ...
                            if(data.resultCode == '1') {
                                 ...
                                 var class_code = $('input:radio[name="chk_info"]:checked' ).val();
      ...
      function memberJoin(){
           ...
           , success     : function(data) {
                      ...
                      if(data.member_join_chk== "Y"){
                           $( "#dialog_a").attr("data-rel", "");
                           $( "#dialog_a").attr("href", "/member/memberLogin.do#two_page" );
                           $( "#dialog_txt").text( "회원가입이 완료되었습니다." );
                           $( "#popupDialog").popup( "open");
                
                           
                     } else if(data.member_join_chk== "R"){
                           $( "#dialog_txt").text( "이미 가입된 회원입니다. 고객센터에 문의주시기 바랍니다." );
                           $( "#popupDialog").popup( "open");
                     }else{
                         $( "#dialog_txt").text( "회원 가입실패..고객센터에 문의주시기 바랍니다." );
                         $( "#popupDialog").popup( "open");
                   }
                 ...
                 , success     : function(data) {
                            ...
                            if(data.kindList!= null && data.kindList.length != 0){
                                ...
                                $.each(data.kindList, function (index, kindList) {
                                      ...
                                      var category_num = Number(kindList.code_sort);
                                     
                                    
                                    
                                      if(category_num == i){//1,4,7,10
                                           
                                           html+= "<div class='ui-block-a'><input class='' id='check"+kindList.category+ "' type='checkbox' name='category' value='"+kindList.category+ "'><label class='' for='check"+kindList.category+ "'>"+kindList.code_name+ "</label></div>" ;
                                           i = i+3;
                                     }
                                     
                                      if(category_num == j){//2,5,8,11
                                           html+= "<div class='ui-block-b'><input class='' id='check"+kindList.category+ "' type='checkbox' name='category' value='"+kindList.category+ "'><label class='' for='check"+kindList.category+ "'>"+kindList.code_name+ "</label></div>" ;
                                           j = j+3;
                                     }
                                     
                                      if(category_num == k){//3,6,9,12
                                           html+= "<div class='ui-block-c'><input class='' id='check"+kindList.category+ "' type='checkbox' name='category' value='"+kindList.category+ "'><label class='' for='check"+kindList.category+ "'>"+kindList.code_name+ "</label></div>" ;
                                           k = k+3;
                                     }
            ...
            $.ajax({
                 ...
                 , success     : function(data) {
                            ...
                            if(data.areaList!= null && data.areaList.length != 0){
                                 ...
                                 var html = "<option value='default' >도를 선택해주세요</option>";
...
function getZip_city(){
            ...
            $.ajax({
                 ...
                 , success     : function(data) {
                            ...
                            if(data.cityList!= null && data.cityList.length != 0){
                                 ...
                                 var html = "<option value='default'  id='default_option'>시/군/구를 선택해주세요</option>";
                                $.each(data.cityList, function (index, cityList) {
                                     html+= " <option value='"+cityList.gugun+ "'>"+cityList.gugun+ "</option>" ;
                                });
                                
                           $( "#cityList").html(html);
                           $( "#cityList-button span").html( "시/군/구를 선택해주세요");
      ...
      function chkEmail(emailStr,chkVal) {
            ...
            if(emailStr.search(format) == -1||emailStr=="OOO@email.com" ){
                $( "#dialog_txt").text( "이메일 형식에 맞게 입력해주세요.");
                $( "#popupDialog").popup( "open");
                chkVal = 0;
                 return chkVal;
           } else{
